...
$(function() {
      var projectid=null;
	$('#dg').datagrid({
		url : '${backstage}/ProjectPantAction_query',
		...
		toolbar : [ {
			...
			handler : function() {
				...
				parent.$('#win').window({
					...
					width: 890,
					height: 600,
					content :'<iframe title="编辑项目" src="${backstage}/project/updateprojectpant.jsp?projectid='+projectid+'" frameborder="0" scrolling="no" width="100%" height="100%" />',
				    modal:true   
				});
			...
			handler : function() {
				parent.$('#win').window({
					...
					width: 890,
					height: 600,
					content :'<iframe src="${backstage}/project/addprojectpant.jsp" frameborder="0" scrolling="no" width="100%" height="100%" />',
				    modal:true   
				});
		  		...
		  		if(rows.length==0){
		  			...
		  			$.messager.confirm('确认','您确认想要冻结这'+rows.length+'记录吗？',function(r){    
		  				 if (r){    
		  			       ...
		  			       $.post("ProjectPantAction_updatestauts",{ids:ids},function(result){
		  			    	   if(result=="true"){
		  			    		   alert("操作成功！");
		  			    		   //取消选中所以行
		  		...
		  		if(rows.length==0){
		  			...
		  			$.messager.confirm('确认','您确认想要删除这'+rows.length+'记录吗？',function(r){    
		  			    if (r){    
		  			       ...
		  			       //拼接id发送给后台
		  			       ids=ids.substring(0,ids.lastIndexOf(","));
		  			       $.post("ProjectPantAction_delete",{ids:ids},function(result){
		  			    	   if(result=="true"){
		  			    		   alert("删除成功！");
		  			    		   //取消选中所以行
		  			    		    $('#dg').datagrid('uncheckAll');
